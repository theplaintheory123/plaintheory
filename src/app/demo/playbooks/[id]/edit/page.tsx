...
export default function EditPlaybookPage({ params }: Props) {
  const { id } = use(params)
  const router = useRouter()

  // Mock initial data
  const [title, setTitle] = useState('Employee Onboarding Process')
  const [description, setDescription] = useState('Complete guide for onboarding new team members to ensure a smooth transition and quick productivity.')
  const [category, setCategory] = useState('HR')
  const [steps, setSteps] = useState<Step[]>([
    { id: '1', title: 'Send Welcome Email', description: 'Send a personalized welcome email to the new employee with first-day instructions.', tools: [{ name: 'Gmail', url: 'https://mail.google.com' }] },
    { id: '2', title: 'Prepare Workstation', description: 'Set up the new employee\'s desk with computer, monitor, keyboard, and necessary office supplies.', tools: [] },
    { id: '3', title: 'Create Accounts', description: 'Create all necessary accounts including email, Slack, project management tools, and HR system access.', tools: [{ name: 'Google Admin', url: 'https://admin.google.com' }, { name: 'Slack Admin', url: 'https://slack.com/admin' }] },
  ])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log({ id, title, description, category, steps })
      ...
      <header className="sticky top-0 z-40 border-b border-slate-200 bg-white">
        <div className="mx-auto flex h-16 max-w-5xl items-center justify-between px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-lg font-semibold text-slate-900">Edit Playbook</h1>
